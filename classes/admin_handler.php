...
class AdminHandler {
	...
	{
		return $this->gh->getID("SELECT `id` FROM `settings` WHERE `using` = 1");
	}

	/*private function getSetAcadAndSemID($settingsID)
	{
		$sql = "SELECT s.`acaid`, s.`semid` 
				FROM `settings` AS s, `academic_year` AS a 
				WHERE s.`using` = 1 AND s.`due_days` > 0 
				AND s.`acaid` = a.`id` AND s.`id` = :s";
		return $this->gh->getData($sql, array(':s' => $settingsID));
	}*/
	
	public function getSettingsStatus()
	{
		$sql = "SELECT `later_date` FROM `setted` WHERE `set_later` = 1";
		$result = $this->gh->getData($sql);
	...
	public function setLater()
	{
		...
		$result = $this->gh->inputData($sql, array(':d' => $date));
		if ($result == 1) {
	...
	{
		return $this->gh->getData("SELECT * FROM `academic_year`");
	}

	public function getSemesters()
	{
		return $this->gh->getData("SELECT * FROM `semester`");
	}

	public function getPrograms()
	{
		return $this->gh->getData("SELECT * FROM `program`");
	}

	public function addAcademicYear($start, $end)
	{
		$sql = "INSERT INTO `academic_year`(`start`, `end`) VALUES(:s, :e)";
		if ($this->gh->inputData($sql, array(":s" => $start, ":e" => $end))) {
	...
	public function addAProgram($program)
	{
		...
		if ($this->gh->inputData($sql, array(":s" => $program))) {
	...
	private function getSetIDByAcaAndSem($acaid, $semid)
	{
		...
		return $this->gh->getID($sql, array(':a' => $acaid, ':s' => $semid));
	}

	private function addQRCodesFromSettingsSetup()
	{
		$results = $this->gh->getData("SELECT * FROM `students` WHERE `deleted` = 0 AND `done` = 0 AND `type` = 2");
		if (!empty($results)) {
	...
	private function getProgramIdByName($program)
	{
		...
		return $this->gh->getID($sql, array(':p' => $program));
	}
	...
	public function getDataStats($status)
	{
		...
		return $this->gh->getData($sql, $param);
	}
	...
	public function resetUserPassw($user_id, $current_pw, $new_pw)
	{
		...
		$id = $this->gh->getID($sql, array(":c" => sha1($current_pw), ":u" => $user_id));
		if ($id) {
	...
	{
		if ($this->gh->getTotalData("SELECT * FROM `students` WHERE deleted = 0 AND `done` = 0 AND `pid` = 1")) {
			$this->gh->inputData("UPDATE `students` SET `done` = 1 WHERE `pid` = 1");
		}
		return 1;
	}
	...
	{
		if ($this->gh->getTotalData("SELECT * FROM `students` WHERE deleted = 0 AND `done` = 0 AND `pid` = 2")) {
			$this->gh->inputData("UPDATE `students` SET `done` = 1 WHERE `sem` = 2 AND `pid` = 2");
			$this->gh->inputData("UPDATE `students` SET `sem` = 2 WHERE `sem` = 1 AND `pid` = 2");
		}
		return 1;
	}
	...
	{
		if ($this->gh->getTotalData("SELECT * FROM `students` WHERE deleted = 0 AND `done` = 0 AND `pid` = 3")) {
			$this->gh->inputData("UPDATE `students` SET `done` = 1 WHERE `sem` = 3 AND `pid` = 3");
			$this->gh->inputData("UPDATE `students` SET `sem` = 3 WHERE `sem` = 2 AND `pid` = 3");
			$this->gh->inputData("UPDATE `students` SET `sem` = 2 WHERE `sem` = 1 AND `pid` = 3");
		}
		return 1;
	}
	...
	public function setupSemester($aca, $semid, $start_date, $end_date, $t_hold, $overwrite, $t_type)
	{
		...
		if ($overwrite == 2) {
			...
			$params = array(':a' => $aca, ':sm' => $semid, 
							':sd' => $start_date, ':ed' => $end_date, 
							':t' => $t_hold, ':tt' => $thresh_type);
			if ($this->gh->getData($sql, $params)) {
			...
			if ($this->getSettingDueStatus() > 0) {
				...
				$this->gh->getData("UPDATE `settings` SET `using` = 0");
				...
				if ($this->gh->inputData($sql, $params)) {
					if ($this->gh->inputData("UPDATE `setted` SET `set_later` = 0 WHERE `id` = 1")) {
